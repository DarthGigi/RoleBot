...
export class ButtonHandler {
  ...
  public static handleButton = async (interaction: ButtonInteraction, args: string[]) => {
    ...
    const member = await interaction.guild?.members.fetch(interaction.user.id).catch((e) => this.log.error(`Fetching user[${interaction.user.id}] threw an error.\n${e}`));
    ...
    if (category.requiredRoleId && !member.roles.cache.has(category.requiredRoleId)) {
      ...
      return interaction.editReply(`Hey! You lack the required role to get anything from this category. ${RolePing(category.requiredRoleId)}`);
    }
    ...
    if (category.excludedRoleId && member.roles.cache.has(category.excludedRoleId)) {
      return interaction.editReply(`Heyo! You have a role that prevents you from obtaining anything from this category. ${RolePing(category.excludedRoleId)}`);
    }

    const handleError = (interaction: ButtonInteraction, error: unknown, type: "add" | "remove") => {
      this.log.debug(`Failed to ${type} role[${reactRole.roleId}] user[${member.id}]\n${error}`);

      return interaction.editReply(`Hey! I couldn't ${type} the role ${RolePing(reactRole.roleId)}. Do I have manage role permissions?`);
    };

    // Remove the role if the user has it. Should catch mutually exclusive removes too.
    if (member.roles.cache.has(reactRole.roleId)) {
      member.roles.remove(reactRole.roleId).catch((e) => handleError(interaction, e, "remove"));

      return interaction.editReply(`Hey! I removed the role ${RolePing(reactRole.roleId)} from you successfully.`);
    }

    if (category.mutuallyExclusive) {
      await ButtonHandler.mutuallyExclusive(interaction, member, category, reactRole);
    } else {
      member.roles.add(reactRole.roleId).catch((e) => handleError(interaction, e, "add"));

      return interaction.editReply(`Hey! I added the role ${RolePing(reactRole.roleId)} to you successfully.`);
    }
  };

  public static mutuallyExclusive = async (interaction: ButtonInteraction, member: GuildMember, category: ICategory, role: ReactRole) => {
    const roles = (await GET_ROLES_BY_CATEGORY_ID(category.id, category.displayOrder)).map((r) => r.roleId);

    const rolesToRemove = member.roles.cache.filter((r) => roles.includes(r.id));
    ...
    await member.roles.add(role.roleId).catch((e) => {
      ...
      return interaction.editReply(`Hey! I couldn't add the role ${RolePing(role.roleId)}. Do I the manage roles permission?`);
    });

    return interaction.editReply(`Hey! I gave you the ${RolePing(role.roleId)} role and removed ${rolesToRemove.map((r) => RolePing(r.id)).join(" ")}`);
  };
}
