...
export default class RoleBot extends Discord.Client {
  ...
  constructor() {
    super({
      intents: [Discord.IntentsBitField.Flags.Guilds, Discord.IntentsBitField.Flags.GuildMembers, Discord.IntentsBitField.Flags.GuildMessageReactions],
    ...
    this.on("ready", (): void => {
      ...
      this.log.debug(`RoleBot reporting for duty. Currently watching ${this.guilds.cache.size} guilds.`);
    ...
    this.on("interactionCreate", async (interaction) => InteractionHandler.handleInteraction(interaction, this));
    ...
    this.on("guildCreate", (guild) => {
      ...
      guildUpdate(guild, "Joined", this).catch((e) => this.log.error(`Failed to send webhook for guild join.\n${e}`));
    });
    this.on("guildDelete", (guild) => {
      ...
      guildUpdate(guild, "Left", this).catch((e) => this.log.error(`Failed to send webhook for guild leave.\n${e}`));
    });
    // React role handling
    this.on("messageReactionAdd", (...r) => {
      this.reactHandler.handleReaction(...r, "add").catch((e) => this.log.error(e));
    });
    this.on("messageReactionRemove", (...r) => {
      this.reactHandler.handleReaction(...r, "remove").catch((e) => this.log.error(e));
    });
    ...
    this.on("roleDelete", async (role) => {
      try {
        ...
        this.log.error(`Failed to delete react role info on role[${role.id}] delete.\n${e}`, role.guild.id);
      }
    });
  }

  private updatePresence = () => {
    if (!this.user) return this.log.error(`Can't set presence due to client user missing.`);
