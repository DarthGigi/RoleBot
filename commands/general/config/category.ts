...
export class CategorySubCommand extends SlashSubCommand {
  constructor(baseCommand: string) {
    super(baseCommand, "category", "Change how categories and their react roles display.", [
      {
        name: "react-type",
      ...
      {
        name: "hide-embed",
        description: "Change if Role Minion uses an embed or just a normal message.",
        type: ApplicationCommandOptionType.Boolean,
      },
    ]);
  }
  ...
  execute = async (interaction: ChatInputCommandInteraction) => {
    ...
    const reactType = parseGuildReactString(reactTypeString as keyof typeof GuildReactType);
